...
// Start server
app.listen(PORT, () => {
  logger.info(`🚀 VeriGrade AI Features Backend API running on port ${PORT}`);
  logger.info(`📊 Environment: ${process.env.NODE_ENV || 'development'}`);
  logger.info(`🔗 Health check: http://localhost:${PORT}/health`);
  logger.info(`📚 API docs: http://localhost:${PORT}/api`);
  logger.info(`🤖 AI Features: ${process.env.OPENAI_API_KEY ? 'Enabled' : 'Mock Mode'}`);
  logger.info(`📄 PDF Generation: Mock Mode`);
  logger.info(`⚡ Queue Worker: ${process.env.REDIS_URL ? 'Configured' : 'Mock Mode'}`);
});
